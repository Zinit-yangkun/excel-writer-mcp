...
def create_workbook(path: str, sheet_name: str = "Sheet1") -> str:
    ...
    if _is_xlsm(path):
        raise ValueError("Cannot create .xlsm from scratch. Use copy_file from an existing .xlsm template.")
...
def copy_file(source_path: str, dest_path: str) -> str:
    ...
    if Path(dest_path).exists():
        raise FileExistsError(f"Destination already exists: {dest_path}. Will not overwrite.")
...
def manage_sheets(
    ...
    try:
        if action == "create":
            ...
            raise ValueError(f"Unknown action '{action}'. Use: create, delete, rename.")
        _save_workbook(wb, path)
    finally:
        wb.close()
...
def write_data(
    ...
    if not Path(path).exists():
        if _is_xlsm(path):
            raise ValueError(".xlsm file must exist before writing. Use copy_file from a template.")
        wb = openpyxl.Workbook()
    else:
        wb = _load_workbook(path)
    try:
...
def modify_rows_columns(
    ...
    try:
        ...
        if not fn:
            raise ValueError(f"Unknown action '{action}'. Use: insert_rows, delete_rows, insert_cols, delete_cols.")
        fn()
        _save_workbook(wb, path)
    finally:
        wb.close()
...
def read_csv(
    ...
    # First pass: count total rows without holding parsed data in memory
    with open(path, newline="", encoding="utf-8-sig") as f:
        total = sum(1 for _ in csv.reader(f))

    # Second pass: read only the requested chunk
    with open(path, newline="", encoding="utf-8-sig") as f:
        reader = csv.reader(f)
        s = max(start_row - 1, 0)
        # Skip rows before the requested start
        for _ in itertools.islice(reader, s):
            pass
        chunk = [
            [_convert_value(cell) for cell in row]
